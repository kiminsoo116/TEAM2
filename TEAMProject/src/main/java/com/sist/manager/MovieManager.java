...
public class MovieManager {
	...
	public static void main(String[] args) {
		try {
			...
			Document doc = Jsoup.connect("https://movie.naver.com/movie/running/current.naver").timeout(30000).get();

			Elements movies = doc.select("dt.tit a");
			Elements posters = doc.select("div.thumb img");
			Elements sold = doc.select("div.b_star span.num");
			...
			for (int i = 0; i < movies.size(); i++) {
				...
				/** 줄거리 **/
				String m_content = doc2.select("div.story_area").text();
