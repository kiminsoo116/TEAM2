...
public class MypageDAO {
	...
	public Movie_ReservationVO reservationListData(String userId) {
		...
		Movie_ReservationVO vo = new Movie_ReservationVO();
		String sql = "SELECT * FROM (SELECT rownum num, mr.* FROM (select movie_reservation.*,movie.m_title, movie.m_poster from movie_reservation join movie on movie_reservation.m_no=movie.m_no where movie_reservation.u_id=? order by movie_reservation.MR_TIMESTAMP DESC) mr) WHERE num=1";
		try {
			getConnection();
			ps = conn.prepareStatement(sql);
			ps.setString(1, userId);
